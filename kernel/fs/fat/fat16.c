...
int fat16_read(struct disk *disk, void *descriptor, uint32_t size, uint32_t nmemb, void *out_ptr);
...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t directory_start_sector)
{
	...
	if (diskstreamer_seek(stream, directory_start_pos) != M_ALL_OK)
	{
		res = -EIO;
		goto out;
	}
	...
	while (1)
	{
		if (diskstreamer_read(stream, &item, sizeof(item)) != M_ALL_OK)
		{
			res = -EIO;
			goto out;
		}
...
int fat16_get_root_directory(struct disk *disk, struct fat_private *fat_private, struct fat_directory *directory)
{
	...
	struct disk_stream *stream = fat_private->directory_stream;
	if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != M_ALL_OK)
	{
		res = -EIO;
		goto err_out;
	}

	if (diskstreamer_read(stream, dir, root_dir_size) != M_ALL_OK)
	{
		res = -EIO;
		goto err_out;
	}
...
int fat16_resolve(struct disk *disk)
{
	...
	int res = 0;
	struct fat_private *fat_private = (struct fat_private*)kalloc();
	//fat_private = kalloc();

	//(sizeof(struct fat_private));
	...
	if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != M_ALL_OK)
	{
		res = -EIO;
		goto out;
	}
	...
	if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != M_ALL_OK)
	{
		res = -EIO;
		goto out;
	}
...
static int fat16_get_fat_entry(struct disk *disk, int cluster)
{
	...
	res = diskstreamer_seek(stream, fat_table_position + (cluster * M_FAT16_FAT_ENTRY_SIZE));
	if (res < 0)
	{
		goto out;
	}
...
static int fat16_read_internal_from_stream(struct disk *disk, struct disk_stream *stream, int cluster, int offset, int total, void *out)
{
	...
	//printk(" cLUSTER TO USE: %d; ", cluster_to_use);
	if (cluster_to_use < 0)
	{
		res = cluster_to_use;
		goto out;
	...
	//printk(" Offset %d ", offset_from_cluster);


	int starting_sector = fat16_cluster_to_sector(private, cluster_to_use);
	//printk(" Starting Sector %d ", starting_sector);

	int starting_pos = (starting_sector * disk->sector_size) + offset_from_cluster;
	//printk(" Starting Posiion %d\n", starting_pos);
...
struct fat_directory *fat16_load_fat_directory(struct disk *disk, struct fat_directory_item *item)
{
	...
	directory = kalloc();//(sizeof(struct fat_directory));
	...
	directory->item = kalloc();//(directory_size);
	if (!directory->item)
	{
		res = -ENOMEM;
		goto out;
...
{
	struct fat_item *f_item = (struct fat_item*)kalloc();//(sizeof(struct fat_item));
	if (!f_item)
	{
		return 0;
	}
...
struct fat_item *fat16_find_item_in_directory(struct disk *disk, struct fat_directory *directory, const char *name)
{
	...
	//	if(directory->item[i].attribute != 0xf)
//		printk("NAME=%s\n", tmp_filename);
...
void *fat16_open(struct disk *disk, struct path_part *path, FILE_MODE mode)
{
	...
	descriptor = (struct fat_file_descriptor*)kalloc();
	if (!descriptor)
	{
		err_code = -ENOMEM;
		goto err_out;
